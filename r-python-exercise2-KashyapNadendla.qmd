...
p_load(DBI, # DBI databases
       ...
       qqplotr,lubricate, car, Hmisc, wavelets, sampling,wordcloud,RColorBrewer)
```
...
dataTab <- read_delim(here("data", "x.tsv"))
...
#split on regular expression pattern ":", this function returns a matrix
names
```
...
#use the weighted average of k most similar samples


data(algae, package="DMwR2") #get data again so there are unknown values
algae <- algae[-manyNAs(algae), ] 
algae <- knnImputation(algae, k = 10, meth="median") 
#use the median of k most similar samples
```
...
## Discreitizing variables (binning)
